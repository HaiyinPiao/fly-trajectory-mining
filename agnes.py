...
plt.title('Flight Trajectory Hierarchical Clustering Dendrogram', fontsize=60)
plt.xlabel('sample index', fontsize=60)
...
plt.title('Flight Trajectory Cluster 2D Scatter Plot', fontsize=60)
plt.xlabel('x east / m ', fontsize=60)
plt.ylabel('y north/ m ', fontsize=60)
total_clust = np.repeat( cluster, TIME_STEP, axis=0 )
plt.scatter(traj_scat[:, 0], traj_scat[:, 1], c=total_clust, cmap='prism' )
...
plt.title('Flight Trajectory Cluster 3D Scatter Plot', fontsize=60)
plt.xlabel('x east / m ', fontsize=60)
plt.ylabel('y north / m ', fontsize=60)
plt.ylabel('z up / m ', fontsize=60)
ax3D = fig.add_subplot( 111, projection='3d' )
ax3D.scatter( traj_scat[:, 0], traj_scat[:, 1], traj_scat[:, 2], c=total_clust, cmap='prism' )
